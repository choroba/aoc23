...
my $disintegrate_tally = 0;
for my $brick_index (0 .. $#bricks) {
    my @supports = keys %{ $supports{$brick_index} };
    my @other_support = map [grep $_ != $brick_index,
                             keys %{ $supported_by{$_} }], @supports;
    my $can_disintegrate = 1;
    for my $higher (0 .. $#supports) {
        undef $can_disintegrate unless @{ $other_support[$higher] };
    }
    ++$disintegrate_tally if $can_disintegrate;
}
say $disintegrate_tally;
